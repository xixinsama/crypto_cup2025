...
const submit = () => {
    isform.value.validate((valid: boolean) => {
        if (valid) {
            ...
            // 即输入到falg2Server.c的Result server()函数
            // 在falg2Server.c查看具体的计算过程, 以及私钥 d2 的值
            // 服务端用服务端私钥d2计算服务端协同签名值 s2, s3, r 发送给客户端
            sign_param_send(data).then((res: any) => {
                // 客户端通过s2, s3, r，计算协同签名值 s
                let str_s: any = clientSign2(smPassword.value, res.s2, res.s3, res.r); // 输入s2, s3, r, 返回smPassword.value, 即s
                if (str_s.errMessage) {
                    ElMessage.error(errMessage)
                    loading.value = false;
                    return
